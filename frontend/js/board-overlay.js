...
async function openTaskBig(id) {
    ...
    let currentTask = tasks.filter(t => t.id == id)[0] ?? null;
    activeTask = currentTask;
    let popUp = document.getElementById('popUpTaskBig');
    // popUp.innerHTML = '';
    // subtasks = [];
    renderAlInformationsTaskBig(popUp, currentTask);
}
 ...
 */
function renderAlInformationsTaskBig(popUp, currentTask) {
    popUp.innerHTML = renderPopUpCardTask(currentTask);
 ...
 */
function renderPopUpCardTask(currentTask) {
    return /*html*/`
        ...
        <div class="pu-big-edit-container">
            <div class="save-subtasks" onclick="saveSubTaskChanges()">
                    <span>Save</span>
                </div>
            <div class="pu-big-seperator"></div>
            <div class="pu-big-edit-only" onclick="deleteTask(${currentTask['id']})">
                <img class="pu-big-delete" src="./assets/img/delete-img.png">
                <span>Delete</span>
            </div>
            <div class="pu-big-seperator"></div>
            <div class="pu-big-edit-only" onclick="editTask(${currentTask['id']})">
    ...
    for (let i = 0; i < currentTask['subtasks'].length; i++) {
        ...
        subtasksBigContainer.innerHTML += /*html*/`
            <div class="pu-big-subtask-container">
                <img id="subtaskImg${i}" src="./assets/img/check-button.png" onclick="renderFinishedSubtaskPu(${currentTask.id}, ${i})">
 ...
 */
async function renderFinishedSubtaskPu(id, num) {
    subtasks = [];
    let currentTask = tasks.filter(t => t.id == id)[0];
    let currentSubtask = currentTask.subtasks[num];
    currentSubtask.isToggled = !currentSubtask.isToggled;
    renderCheckImage(currentSubtask, num);
    delete currentSubtask.parent;
    subTasks.push(currentSubtask);
    onSubTaskToggle();
}

function onSubTaskToggle() {
    const el = document.getElementById("popUpTaskBig");
    el.classList.toggle("saveRequested", subTasks.length > 0);
}

async function saveSubTaskChanges() {
    for (const subtask of subTasks) {
        await patchSubTask(subtask);
    }
    subTasks = [];
}
