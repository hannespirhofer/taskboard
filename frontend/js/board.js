 ...
 */
async function updateBoardTask(taskid, newprogress) {
    let t = null;
    tasks.forEach(task => {
        if (task.id == taskid) {
            t = task;
    ...
    try {
        if (!t) throw new Error('No task given');
        await patchTask(t);
        // renderBoard();
    } catch (error) {
        console.error(error);
...
function renderTasks(tasks, id) {
    ...
    tasks.forEach((task) => {
        ...
        // const coworkersHTML = '<div class="none">coworkersHTML</div>';
        // let coworkerIds = task['contactids'];
        const coworkersHTML = collectAndRenderCoworkers(task['contactids'] ?? []);
        const [toggled, subtasksQty] = getSubTaskDetails(task.subtasks);
 ...
 */
function getSubTaskDetails(arr) {
    if (!arr) return [0,0]
    let x = 0;
    arr.forEach(st => {
        x += st.isToggled ? 1 : 0;
...
function renderTaskHtml(task, subtasksQty, toggled, coworkersHTML, prioHtml) {
    return `
    <div id="${task['id']}" class="todo" draggable="true" ondragstart="drag(event)" onclick="openTaskBig(${task['id']})">
