...
function addSubtaskEdit() {
    ...
    if (subtask !== '') {
        ...
        activeTask.subtasks.push(currentSubtask);
        renderSubtasksEdit();
    }
    input.value = '';
...
function renderSubtasksEdit() {
    ...
    for (let i = 0; i < subtasks.length; i++) {
        subtasksContainer.innerHTML += returnSubtaskTemplateEdit(subtasks[i]);
    }
    // for (let i = 0; i < subtasks.length; i++) {
 ...
 */
function returnSubtaskTemplateEdit(subtask) {
    const i = subtask.id;
    return /*html*/`
        <div class="added-subtask-container" id="addedSubtaskContainerEdit${i}">
            <input class="added-subtask" value="${subtask.name}" id="addedSubtaskEdit${i}" type="text" ondblclick="inputAddedSubtaskEdit(${i})" readonly>
...
function deleteAddedSubtaskEdit(i) {
    const idx = activeTask.subtasks.find(st => st.id == i);
    activeTask.subtasks.splice(idx, 1);
    // subtasks.splice(i, 1);
    renderSubtasksEdit();
}
