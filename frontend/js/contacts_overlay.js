 ...
 * @param {String} phone
 */
async function buildContact(firstname, lastname, fullname, email, phone) {
    // let initials = createInitials(firstname, lastname);
    // let id = createNewId();
    // setandSaveContact(id, email, firstname, lastname, fullname, initials, phone);
    const res = await saveContact(getContact(firstname, lastname, email, phone));
    if (res && res.contactid) renderAndShowContact(res.contactid)
}

function getContact(firstname, lastname, email, phone) {
    return {
        firstname: firstname,
        lastname: lastname,
        email: email,
 ...
 * @param {String} phone
 */
async function setandSaveContact(id, email, firstname, lastname, fullname, initials, phone) {
    const contact = {
        "contactid": id,
        "firstname": firstname,
        "lastname": lastname,
 ...
 */
function renderAndShowContact(id) {
 ...
 * @param {*} phone
 */
async function generateSaveAndReload(index, id, fullname, firstname, lastname, email, phone) {
 ...
 * @param {String} phone
 */
function _saveContact(index, firstname, lastname, fullname, email, phone) {
    if (index) {
...
function returnDeleteHtml(contact, index) {
    return `
        <div class="inner-box full-height">
            ...
            <button class="final-delete-btn button-dark" onclick="finalDeleteContact(${contact.contactid})">Yes, delete!</button>
        </div>
    `;
}
 ...
 */
async function finalDeleteContact(id) {
    try {
        ...
        console.error('Couldnt delete the contact with the id: ', id);
        renderContactList();
    }
}
